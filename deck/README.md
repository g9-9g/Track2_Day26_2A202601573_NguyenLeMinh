...
**`deck/lineup.json`** plays eight attacks and two blanks in this order:

```
atk_05 (schema_bomb) → blk_01 → atk_01 (shadow) → atk_08 (forged_card) →
atk_03 (drift) → blk_02 → atk_06 (header_spoof) → atk_02 (replica_flip) →
atk_09 (faithless peer) → atk_07 (identity/act)
```

The two blanks probe over-defensive gateways and false-positive prosecutors. The
low-hit `atk_04`/`atk_10` routes are benched; the strongest infrastructure and
identity attacks remain late, where round scaling is highest.
